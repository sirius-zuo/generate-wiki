...
After this round the run is fully autonomous, with no further questions
in generate mode.
...
each page's review outcome, and, as a first-class output rather than an
aside, the **real project findings** list (dead code, unwired features, stale
...
two changes: the brief is a **refresh brief**, meaning the drift evidence
from R1 (the commits/PRs that touched the page's anchors, plus `gh pr
view` bodies when available) instead of a from-scratch source table; and the
