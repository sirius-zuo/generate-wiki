...
`<<BRIEF_PATH>>` is the same page brief given to the implementer.
`<<REPORT_PATH>>` is the implementer's report file (sections written, Key
Decisions with Refs, symbols verified, check output, self-review notes).
`<<DIFF_PACKAGE_PATH>>` is a file containing the implementer's diff:
...
is the full contents of `references/binding-rules.md`, pasted verbatim.
For the reviewer this is the attention lens, not just a rule list: every
finding should trace back to one of these rules or to a factual mismatch
with source. `<<MIN_COVERAGE>>` restates the brief's minimum flows and
