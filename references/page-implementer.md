...
`<<BRIEF_PATH>>` — the path to the page brief you composed for this page
(source table: directories/modules, design docs, PRs/commits, minimum
flows and decisions). `<<WIKI_DIR>>` — the target repo's wiki directory
(e.g. `wiki/`). `<<PAGE_FILE>>` — the page's filename inside
`<<WIKI_DIR>>` (e.g. `agent.md`). `<<STYLE_REF>>` — the filename of an
...
against this page. `<<BINDING_RULES>>` — paste the full contents of
`references/binding-rules.md` here, verbatim, unsummarized.
`<<REPORT_PATH>>` — the file this subagent must write its full report to.
...
- `<<WIKI_DIR>>/TEMPLATE.md` — the page structure you must follow
  exactly: 8 sections, in this order, none added and none dropped.
- `<<STYLE_REF>>` — an already-committed page to match for tone,
