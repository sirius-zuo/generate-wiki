...
against the wiki. `<<BINDING_RULES>>` — paste the full contents of
`references/binding-rules.md` here, verbatim, unsummarized.
`<<REPORT_PATH>>` — the file this subagent must write its full report
...
— never mid-run, since the link graph and hub index are only meaningful
