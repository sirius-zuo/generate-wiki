...
against the wiki. `<<DIFF_PACKAGE_PATH>>` is a file containing the whole
...
`<<LEDGER_PATH>>` is the run's ledger (`.generate-wiki/progress.md`):
per-page outcomes and deferred minors recorded as each page completed.
`<<TRIAGE_PATH>>` is a file listing every deferred Minor from the ledger
...
7. **Minor triage:** for every deferred Minor listed in
   `<<TRIAGE_PATH>>`, decide fix-before-merge or leave, with one line of
